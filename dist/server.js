...
app.get('/api/info', (req, res) => {
    ...
    res.json({
        ...
        hasKeyConfigured: Boolean(process.env.ZHIPU_API_KEY || process.env.OPENROUTER_API_KEY || process.env.OPENAI_API_KEY),
    ...
    try {
        ...
        try {
            ...
            if (err?.message?.includes('generateLegacy') && typeof zaiAgent.generateLegacy === 'function') {
                response = await zaiAgent.generateLegacy(finalPrompt);
            }
            else {
                throw err;
            }
