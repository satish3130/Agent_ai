...
const require_workspace_skills = require("./workspace-skills-4sQ19tXz.cjs");
...
var ProcessHandle = class {
	...
	*/
	...
	get writer() {
		if (!this._writer) this._writer = new stream.Writable({ write: (chunk, _encoding, cb) => {
...
const executeCommandInputSchema = zod_v4.z.object({
	...
	timeout: zod_v4.z.preprocess((value) => {
		if (typeof value !== "string") return value;
		const trimmed = value.trim();
		return NUMERIC_TIMEOUT_STRING_REGEX.test(trimmed) ? Number(trimmed) : value;
	}, zod_v4.z.number()).nullish().describe("Maximum execution time in seconds. Example: 60 for 1 minute."),
	cwd: zod_v4.z.string().nullish().describe("Working directory for the command"),
	tail: zod_v4.z.number().nullish().describe(`For foreground commands: limit output to the last N lines, similar to tail -n. Defaults to 200. Use 0 for no limit.`)
});
...
Use this after starting a background command with execute_command (background: true) to check if the process is still running and read its output.`,
	...
	inputSchema: zod_v4.z.object({
		...
		tail: zod_v4.z.number().optional().describe(`Number of lines to return, similar to tail -n. Positive or negative returns last N lines from end. Defaults to 200. Use 0 for no limit.`),
...
//# sourceMappingURL=workspace-eN4505hM.cjs.map