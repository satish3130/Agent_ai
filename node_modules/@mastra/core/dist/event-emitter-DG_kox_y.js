...
var EventEmitterPubSub = class extends PubSub {
	...
	*/
	...
	async subscribe(topic, cb, options) {
		...
		if (options?.group) this.subscribeWithGroup(topic, cb, options.group);
			...
			const wrapper = (event) => {
				Promise.resolve(cb(event, NOOP_ACK, NOOP_ACK)).catch((err) => {
	...
	*/
	...
	deliverToGroup(topic, group, listenerKey, event) {
		...
		else Promise.resolve(member(eventWithAttempt, ack, nack)).catch((err) => {
			this.logSubscriberError(topic, err);
		});
	}
...
//#endregion
export { isLeaseProvider as i, NoopLeaseProvider as n, PubSub as r, EventEmitterPubSub as t };

//# sourceMappingURL=event-emitter-DG_kox_y.js.map