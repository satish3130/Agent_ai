...
export interface ToolHookContext<TInput = unknown, TContext = unknown, TMetadata extends Record<string, unknown> = Record<string, unknown>> {
...
export interface ToolAfterHookContext<TInput = unknown, TOutput = unknown, TContext = unknown, TMetadata extends Record<string, unknown> = Record<string, unknown>> extends ToolHookContext<TInput, TContext, TMetadata> {
...
export interface ToolHooks<TInput = unknown, TOutput = unknown, TContext = unknown, TMetadata extends Record<string, unknown> = Record<string, unknown>> {
    beforeToolCall?: (context: ToolHookContext<TInput, TContext, TMetadata>) => void | ToolBeforeHookResult<TOutput> | Promise<void | ToolBeforeHookResult<TOutput>>;
 ...
 */
export type ToolExecuteContext<TContext, TRequestContext extends Record<string, any> | unknown = unknown> = Omit<TContext, 'requestContext'> & {
    requestContext: RequestContext<TRequestContext>;
};
/** The `execute` callback signature used by `createTool` and `new Tool(...)`. */
export type ToolExecuteFunction<TSchemaIn, TSchemaOut, TContext, TRequestContext extends Record<string, any> | unknown = unknown> = (inputData: TSchemaIn, context: ToolExecuteContext<TContext, TRequestContext>) => Promise<TSchemaOut | ValidationError | void>;
