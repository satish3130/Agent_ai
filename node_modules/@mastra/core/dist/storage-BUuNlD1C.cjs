...
require("./request-context-ByoZMp-j.cjs");
...
var AgentThreadStreamRuntime = class {
	...
	*/
	...
	#watchThreadRunCompletion(state, pubsub, key, record, registered) {
	...
	*/
	...
	async #waitForRemoteRunToFinish(pubsub, key, runId) {
		...
		const onEvent = async (event, ack) => {
			...
			const isTerminal = (data?.type === "run-completed" || data?.type === "run-aborted" || data?.type === "run-failed") && data.runId === runId;
			try {
				await ack?.();
			} catch {}
			if (isTerminal) {
...
//# sourceMappingURL=storage-BUuNlD1C.cjs.map