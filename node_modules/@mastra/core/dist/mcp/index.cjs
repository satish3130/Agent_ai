...
*/
var MCPServerBase = class extends require_base.MastraBase {
	...
	__registerMastra(mastra) {
		...
		if (this.originalTools && typeof this.originalTools === "object") Object.entries(this.originalTools).forEach(([key, tool]) => {
			try {
				...
				if (!(error instanceof require_error.MastraError) || error.id !== "MASTRA_ADD_TOOL_DUPLICATE_KEY") throw error;
			}
		});
		if (this.agents && typeof this.agents === "object") Object.entries(this.agents).forEach(([key, agent]) => {
			try {
				...
				if (!(error instanceof require_error.MastraError) || error.id !== "MASTRA_ADD_AGENT_DUPLICATE_KEY") throw error;
			}
		});
		if (this.workflows && typeof this.workflows === "object") Object.entries(this.workflows).forEach(([key, workflow]) => {
			try {
				...
				if (!(error instanceof require_error.MastraError) || error.id !== "MASTRA_ADD_WORKFLOW_DUPLICATE_KEY") throw error;
			}
		});
	}
	/**
	...
	*/
	constructor(config) {
		super({
			component: require_logger.RegisteredLogger.MCP_SERVER,
			name: config.name
		});
		this.name = config.name;
		this.version = config.version;
		if (config.id) {
			this._id = require_slugify.slugify(config.id);
			this.idWasSet = true;
		} else this._id = this.mastra?.generateId() || (0, crypto.randomUUID)();
